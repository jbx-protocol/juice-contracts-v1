...
contract FundingCycles is TerminalUtility, IFundingCycles {
    ...
    */
	function currentOf(uint256 _projectId) external view override returns (FundingCycle memory fundingCycle) {
		...
		if (_fundingCycleId > 0) {
			...
			if (_fundingCycle.start <= block.timestamp && _isApproved(_fundingCycle)) return _fundingCycle;
    ...
    */
		...
		returns (FundingCycle memory fundingCycle)
	{
		...
		require(_amount <= _targetOf[fundingCycleId] - _tapped, 'FundingCycles::tap: INSUFFICIENT_FUNDS');
    ...
    */
	function _configurable(
		...
		if (latestIdOf[_projectId] == 0) return _init(_projectId, _getStruct(0), block.timestamp, false);
    ...
    */
	function _tappable(uint256 _projectId) private returns (uint256 fundingCycleId) {
		...
		if (fundingCycleId > 0) {
			...
			if (_fundingCycle.start <= block.timestamp && _isApproved(_fundingCycle)) return fundingCycleId;
		...
		fundingCycleId = _init(_projectId, _fundingCycle, block.timestamp - _timeFromImmediateStartMultiple, true);
	}
    ...
    */
		...
		returns (FundingCycle memory)
	{
		...
		if (_allowMidCycle && _baseFundingCycle.duration > 0) {
			...
			if (_baseFundingCycle.cycleLimit > 0 && _cycleEnd < block.timestamp) {
				_timeFromImmediateStartMultiple = _latestPermanentFundingCycle.duration == 0
					? 0
					: ((block.timestamp - _cycleEnd) % (_latestPermanentFundingCycle.duration * SECONDS_IN_DAY));
			} else {
		...
		FundingCycle memory _fundingCycleToCopy = _cycleLimit == 0 ? _latestPermanentFundingCycle : _baseFundingCycle;
    ...
    */
	function _updateFundingCycle(
		...
		uint256 _start = _deriveStart(_baseFundingCycle, _latestPermanentFundingCycle, _mustStartOnOrAfter);
    ...
    */
	function _deriveStart(
		...
		if (_mustStartOnOrAfter <= _baseFundingCycle.start + _durationInSeconds * _cycleLimit) {
			...
			_timeFromImmediateStartMultiple = (_mustStartOnOrAfter - _nextImmediateStart) % _durationInSeconds;
		} else {
		...
		if (_limitLength == 0 || _limitLength > _startDistance) {
			...
			if (_latestPermanentFundingCycle.discountRate > 0) {
				...
				uint256 _permanentDiscountMultiple = _latestPermanentFundingCycle.duration == 0
					? 0
					: (_startDistance - _limitLength) / (_latestPermanentFundingCycle.duration * SECONDS_IN_DAY);
    ...
    */
	function _deriveNumber(
		...
		if (_limitLength == 0 || _limitLength > _startDistance) {
			...
			number = _baseFundingCycle.number + (_startDistance / (_baseFundingCycle.duration * SECONDS_IN_DAY));
		} else {
			// If the time between the base start at the given start is longer than
			...
			number = _baseFundingCycle.number + (_limitLength / (_baseFundingCycle.duration * SECONDS_IN_DAY));
    ...
    */
	function _isApproved(FundingCycle memory _fundingCycle) private view returns (bool) {
		return _ballotState(_fundingCycle.id, _fundingCycle.configured, _fundingCycle.basedOn) == BallotState.Approved;
	}
