...
contract TerminalV1 is Operatable, ITerminalV1, ITerminal, ReentrancyGuard {
    ...
    */
	function deploy(
		...
		if (_payoutMods.length > 0) modStore.setPayoutMods(_projectId, _fundingCycle.configured, _payoutMods);

		// Set ticket mods if there are any.
		if (_ticketMods.length > 0) modStore.setTicketMods(_projectId, _fundingCycle.configured, _ticketMods);
	}
    ...
    */
	function printPreminedTickets(
		...
		require(
			_processedTicketTrackerOf[_projectId] < 0 ||
				uint256(_processedTicketTrackerOf[_projectId]) + uint256(_weightedAmount) <= uint256(type(int256).max),
			'TerminalV1::printTickets: INT_LIMIT_REACHED'
		);

		_processedTicketTrackerOf[_projectId] = _processedTicketTrackerOf[_projectId] + int256(_weightedAmount);

		// Set the count of preconfigure tickets this project has printed.
		_preconfigureTicketCountOf[_projectId] = _preconfigureTicketCountOf[_projectId] + _weightedAmount;
    ...
    */
	function tap(
		...
		uint256 _feeAmount = _fundingCycle.fee > 0
			? _takeFee(_tappedWeiAmount, _fundingCycle.fee, _projectOwner, string(bytes.concat('Fee from @', _handle)))
			: 0;
    ...
    */
	function printReservedTickets(uint256 _projectId) public override returns (uint256 amount) {
		...
		if (_leftoverTicketAmount > 0) ticketBooth.print(_owner, _projectId, _leftoverTicketAmount, false);

		emit PrintReserveTickets(_fundingCycle.id, _projectId, _owner, amount, _leftoverTicketAmount, msg.sender);
	}
    ...
    */
	function _distributeToPayoutMods(
		...
		PayoutMod[] memory _mods = modStore.payoutModsOf(_fundingCycle.projectId, _fundingCycle.configured);
		...
		for (uint256 _i = 0; _i < _mods.length; _i++) {
			...
			if (_modCut > 0) {
				...
				if (_mod.allocator != IModAllocator(address(0))) {
					...
					if (_terminal == this) {
						...
						_terminal.pay{value: _modCut}(_mod.projectId, _mod.beneficiary, _memo, _mod.preferUnstaked);
					}
				} else {
			...
			emit DistributeToPayoutMod(_fundingCycle.id, _fundingCycle.projectId, _mod, _modCut, msg.sender);
		}
	}
    ...
    */
		...
		returns (uint256 leftoverAmount)
	{
		...
		TicketMod[] memory _mods = modStore.ticketModsOf(_fundingCycle.projectId, _fundingCycle.configured);
		...
		for (uint256 _i = 0; _i < _mods.length; _i++) {
			...
			if (_modCut > 0) ticketBooth.print(_mod.beneficiary, _fundingCycle.projectId, _modCut, _mod.preferUnstaked);

			// Subtract from the amount to be sent to the beneficiary.
			leftoverAmount = leftoverAmount - _modCut;

			emit DistributeToTicketMod(_fundingCycle.id, _fundingCycle.projectId, _mod, _modCut, msg.sender);
		}
	}
    ...
    */
	function _pay(
		...
		uint256 _weight = _fundingCycle.number == 0 ? fundingCycles.BASE_WEIGHT() : _fundingCycle.weight;
		...
		uint256 _reservedRate = _fundingCycle.number == 0 ? 0 : uint256(uint8(_fundingCycle.metadata >> 8));
		...
		if (_unreservedWeightedAmount > 0) {
			...
			// Print the project's tickets for the beneficiary.
			ticketBooth.print(_beneficiary, _projectId, _unreservedWeightedAmount, _preferUnstakedTickets);
    ...
    */
	function _reservedTicketAmountFrom(
		...
		return PRBMath.mulDiv(_unprocessedTicketBalanceOf, 200, 200 - _reservedRate) - _unprocessedTicketBalanceOf;
	}
