...
contract Shwotime is JuiceboxProject {
	...
	function buyTicket(uint256 id, address[] calldata addresses) external {
		require(id > 0 && id <= ticketsCount, 'Shwotime::buyTickets: NOT_FOUND');

		//Mark the msg.sender as committed to buying.
		Tix storage _tickets = tickets[id];

		require(_tickets.expiry > block.timestamp, 'Shwotime::buyTickets: EXPIRED');
		require(_tickets.max >= _tickets.sold, 'Shwotime::buyTickets: SOLD_OUT');

		bool _transferFundsFromMsgSender = true;
		for (uint256 _i = 0; _i < addresses.length; _i++) {
			address _address = addresses[_i];
			if (!_tickets.committed[_address]) _transferFundsFromMsgSender = false;
			if (_tickets.paid[_address]) continue;
			// Nest once.
			bool _transferFundsFromDependency = true;
			for (uint256 _j = 0; _j < _tickets.dependencies[_address].length; _j++) {
				address _subAddress = _tickets.dependencies[_address][_j];
				if (_subAddress != msg.sender && !_tickets.committed[_subAddress]) _transferFundsFromDependency = false;
			}
	...
	function collect(uint256 _id, string calldata _memo) external {
		...
		require(msg.sender == _tickets.owner, 'Shwotime::collect: UNAUTHORIZED');

		require(_tickets.expiry <= block.timestamp, 'Shwotime::collect: TOO_SOON');
