...
contract Projects is ERC721, IProjects, Operatable {
    ...
    */
	function create(
		...
		require(
			projectFor[_handle] == 0 && transferAddressFor[_handle] == address(0),
		...
		if (_terminal != ITerminal(address(0))) _terminal.terminalDirectory().setTerminal(count, _terminal);
    ...
    */
	...
	{
		require(_newHandle != bytes32(0), 'Projects::transferHandle: EMPTY_HANDLE');

		require(
			projectFor[_newHandle] == 0 && transferAddressFor[_handle] == address(0),
    ...
    */
	function challengeHandle(bytes32 _handle) external {
		...
		require(projectFor[_handle] > 0, 'Projects::challenge: HANDLE_NOT_TAKEN');
