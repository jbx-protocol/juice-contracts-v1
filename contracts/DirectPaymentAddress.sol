...
contract DirectPaymentAddress is IDirectPaymentAddress {
    ...
    */
	...
	receive() external payable {
		...
		address _storedBeneficiary = terminalDirectory.beneficiaryOf(msg.sender);
		// If no beneficiary is configured, use the sender's address.
		address _beneficiary = _storedBeneficiary != address(0) ? _storedBeneficiary : msg.sender;

		bool _preferUnstakedTickets = terminalDirectory.unstakedTicketsPreferenceOf(msg.sender);
		...
		emit Forward(msg.sender, projectId, _beneficiary, msg.value, memo, _preferUnstakedTickets);
	}
}
