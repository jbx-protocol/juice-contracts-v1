...
contract TicketBooth is TerminalUtility, Operatable, ITicketBooth {
    ...
    */
	function totalSupplyOf(uint256 _projectId) external view override returns (uint256 supply) {
		...
		if (_tickets != ITickets(address(0))) supply = supply + _tickets.totalSupply();
	}
    ...
    */
	function balanceOf(address _holder, uint256 _projectId) external view override returns (uint256 balance) {
		...
		if (_ticket != ITickets(address(0))) balance = balance + _ticket.balanceOf(_holder);
	}
    ...
    */
	...
	) external override requirePermission(projects.ownerOf(_projectId), _projectId, Operations.Issue) {
		// There must be a name.
		require((bytes(_name).length > 0), 'TicketBooth::issue: EMPTY_NAME');

		// There must be a symbol.
		require((bytes(_symbol).length > 0), 'TicketBooth::issue: EMPTY_SYMBOL');

		// Only one ERC20 ticket can be issued.
		require(ticketsOf[_projectId] == ITickets(address(0)), 'TicketBooth::issue: ALREADY_ISSUED');
    ...
    */
	function print(
		...
		bool _shouldUnstakeTickets = _preferUnstakedTickets && _tickets != ITickets(address(0));
		...
		emit Print(_holder, _projectId, _amount, _shouldUnstakeTickets, _preferUnstakedTickets, msg.sender);
	}
    ...
    */
	function redeem(
		...
		uint256 _unlockedStakedBalance = stakedBalanceOf[_holder][_projectId] - lockedBalanceOf[_holder][_projectId];

		// Get a reference to the number of tickets there are.
		uint256 _unstakedBalanceOf = _tickets == ITickets(address(0)) ? 0 : _tickets.balanceOf(_holder);
		...
		require(
			(_amount < _unstakedBalanceOf && _amount < _unlockedStakedBalance) ||
				...
				(_amount >= _unlockedStakedBalance && _unstakedBalanceOf >= _amount - _unlockedStakedBalance),
		...
		if (_unstakedBalanceOf == 0) {
			...
			_unstakedTicketsToRedeem = _unstakedBalanceOf >= _amount ? _amount : _unstakedBalanceOf;
			// Otherwise, redeem staked tickets before unstaked tickets.
		} else {
			_unstakedTicketsToRedeem = _unlockedStakedBalance >= _amount ? 0 : _amount - _unlockedStakedBalance;
		}
		...
		// Redeem the tickets.
		if (_unstakedTicketsToRedeem > 0) _tickets.redeem(_holder, _unstakedTicketsToRedeem);
		if (_stakedTicketsToRedeem > 0) {
			// Reduce the holders balance and the total supply.
			stakedBalanceOf[_holder][_projectId] = stakedBalanceOf[_holder][_projectId] - _stakedTicketsToRedeem;
			stakedTotalSupplyOf[_projectId] = stakedTotalSupplyOf[_projectId] - _stakedTicketsToRedeem;
		}

		emit Redeem(_holder, _projectId, _amount, _unlockedStakedBalance, _preferUnstaked, msg.sender);
	}
	...
	function stake(
		...
		uint256 _projectId,
		uint256 _amount
	) external override requirePermissionAllowingWildcardDomain(_holder, _projectId, Operations.Stake) {
		// Get a reference to the project's ERC20 tickets.
		ITickets _tickets = ticketsOf[_projectId];

		// Tickets must have been issued.
		require(_tickets != ITickets(address(0)), 'TicketBooth::stake: NOT_FOUND');
	...
	function unstake(
		...
		uint256 _projectId,
		uint256 _amount
	) external override requirePermissionAllowingWildcardDomain(_holder, _projectId, Operations.Unstake) {
		// Get a reference to the project's ERC20 tickets.
		ITickets _tickets = ticketsOf[_projectId];

		// Tickets must have been issued.
		require(_tickets != ITickets(address(0)), 'TicketBooth::unstake: NOT_FOUND');
		...
		require(_unlockedStakedTickets >= _amount, 'TicketBooth::unstake: INSUFFICIENT_FUNDS');
    ...
    */
	function lock(
		...
		uint256 _projectId,
		uint256 _amount
	) external override requirePermissionAllowingWildcardDomain(_holder, _projectId, Operations.Lock) {
		...
		lockedBalanceBy[msg.sender][_holder][_projectId] = lockedBalanceBy[msg.sender][_holder][_projectId] + _amount;
    ...
    */
	function unlock(
		...
		require(lockedBalanceBy[msg.sender][_holder][_projectId] >= _amount, 'TicketBooth::unlock: INSUFFICIENT_FUNDS');
    ...
    */
	function transfer(
		...
		uint256 _amount,
		address _recipient
	) external override requirePermissionAllowingWildcardDomain(_holder, _projectId, Operations.Transfer) {
		// Can't transfer to the zero address.
		require(_recipient != address(0), 'TicketBooth::transfer: ZERO_ADDRESS');
		...
		require(_amount <= _unlockedStakedTickets, 'TicketBooth::transfer: INSUFFICIENT_FUNDS');
