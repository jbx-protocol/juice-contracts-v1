...
abstract contract JuiceboxProject is IERC721Receiver, Ownable {
    ...
    */
	function pay(
		...
		require(_terminal != ITerminal(address(0)), 'JuiceboxProject::pay: TERMINAL_NOT_FOUND');

		_terminal.pay{value: msg.value}(projectId, _beneficiary, _memo, _preferUnstakedTickets);
	}
    ...
    */
	...
	function setOperators(
		...
		_operatorStore.setOperators(_operators, _projectIds, _permissionIndexes);
	}
    ...
    */
	function _takeFee(
		...
		// There must be a terminal.
		require(_terminal != ITerminal(address(0)), 'JuiceboxProject::takeFee: TERMINAL_NOT_FOUND');

		// There must be enough funds in the contract to take the fee.
		require(address(this).balance >= _amount, 'JuiceboxProject::takeFee: INSUFFICIENT_FUNDS');

		// Send funds to the terminal.
		_terminal.pay{value: _amount}(projectId, _beneficiary, _memo, _preferUnstakedTickets);
	}
}
