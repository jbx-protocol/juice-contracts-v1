...
contract ModStore is IModStore, Operatable, TerminalUtility {
    ...
    */
		...
		)
	{
		...
		PayoutMod[] memory _currentMods = _payoutModsOf[_projectId][_configuration];
		...
		for (uint256 _i = 0; _i < _currentMods.length; _i++) {
			if (block.timestamp < _currentMods[_i].lockedUntil) {
				...
				require(_includesLocked, 'ModStore::setPayoutMods: SOME_LOCKED');
			}
		}
		...
		for (uint256 _i = 0; _i < _mods.length; _i++) {
			...
			require(_mods[_i].percent > 0, 'ModStore::setPayoutMods: BAD_MOD_PERCENT');

			// Add to the total percents.
			_payoutModPercentTotal = _payoutModPercentTotal + _mods[_i].percent;

			// The total percent should be less than 10000.
			require(_payoutModPercentTotal <= 10000, 'ModStore::setPayoutMods: BAD_TOTAL_PERCENT');

			// The allocator and the beneficiary shouldn't both be the zero address.
			require(
				_mods[_i].allocator != IModAllocator(address(0)) || _mods[_i].beneficiary != address(0),
			...
			emit SetPayoutMod(_projectId, _configuration, _mods[_i], msg.sender);
		}
	}
    ...
    */
		...
		)
	{
		...
		TicketMod[] memory _projectTicketMods = _ticketModsOf[_projectId][_configuration];
			...
			if (block.timestamp < _projectTicketMods[_i].lockedUntil) {
				...
				for (uint256 _j = 0; _j < _mods.length; _j++) {
					...
					if (
						...
						_mods[_j].beneficiary == _projectTicketMods[_i].beneficiary &&
						// Allow lock extensions.
						_mods[_j].lockedUntil >= _projectTicketMods[_i].lockedUntil
					) _includesLocked = true;
				}
				require(_includesLocked, 'ModStore::setTicketMods: SOME_LOCKED');
			}
		}
		// Delete from storage so mods can be repopulated.
		...
		for (uint256 _i = 0; _i < _mods.length; _i++) {
			// The percent should be greater than 0.
			require(_mods[_i].percent > 0, 'ModStore::setTicketMods: BAD_MOD_PERCENT');

			// Add to the total percents.
			_ticketModPercentTotal = _ticketModPercentTotal + _mods[_i].percent;
			// The total percent should be less than 10000.
			require(_ticketModPercentTotal <= 10000, 'ModStore::setTicketMods: BAD_TOTAL_PERCENT');

			// The beneficiary shouldn't be the zero address.
			require(_mods[_i].beneficiary != address(0), 'ModStore::setTicketMods: ZERO_ADDRESS');

			// Push the new mod into the project's list of mods.
			_ticketModsOf[_projectId][_configuration].push(_mods[_i]);

			emit SetTicketMod(_projectId, _configuration, _mods[_i], msg.sender);
		}
	}
}
