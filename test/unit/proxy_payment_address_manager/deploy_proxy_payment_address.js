			...
			it(successTest.description, async function () {
				...
				await Promise.all(
					ops.map(async (op) => {
						...
						const tx = await this.contract.connect(caller).deploy(op.projectId, op.memo);
	...
	describe('Failure cases', function () {
		tests.failure.forEach(function (failureTest) {
			it(failureTest.description, async function () {
				...
				await expect(this.contract.connect(caller).deploy(projectId, memo)).to.be.revertedWith(revert);
			});
		});
	});
}
