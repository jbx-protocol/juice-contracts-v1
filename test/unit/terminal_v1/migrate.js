	...
	failure: [
		{
			...
			fn: async ({ deployer, deployMockLocalContractFn, mockContracts, governance, targetContract, addrs }) => {
				return {
					caller: deployer,
					projectOwner: addrs[0].address,
					governance,
		...
		},
		{
			...
			fn: async ({ deployer, deployMockLocalContractFn, mockContracts, governance, targetContract }) => {
				return {
					caller: deployer,
					projectOwner: deployer.address,
					governance,
		...
		tests.success.forEach(function (successTest) {
			it(successTest.description, async function () {
				const {
					...
					projectId,
					terminal,
					setup: { unprintedReservedTicketAmount = BigNumber.from(0), permissionFlag, addToBalance = 0 } = {},
				...
				const tx = await this.targetContract.connect(caller).migrate(projectId, terminal.address);
	...
	describe('Failure cases', function () {
		tests.failure.forEach(function (failureTest) {
			it(failureTest.description, async function () {
				...
				// Execute the transaction.
				await expect(
					this.targetContract.connect(caller).migrate(projectId, terminal.address),
