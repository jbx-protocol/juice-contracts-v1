	...
	describe('Success cases', function () {
		tests.success.forEach(function (successTest) {
			it(successTest.description, async function () {
				...
				if (addToBalance) {
					await this.targetContract.connect(caller).addToBalance(projectId, { value: addToBalance });
				}

				// Execute the transaction.
				const tx = await this.targetContract.connect(caller).addToBalance(projectId, { value: amount });
