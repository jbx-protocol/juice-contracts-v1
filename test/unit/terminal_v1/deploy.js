	...
	failure: [
		...
		},
		{
			...
			fn: ({ deployer }) => ({
				...
				payoutMods: [],
				ticketMods: [],
				revert: 'TerminalV1::_validateAndPackFundingCycleMetadata: BAD_RECONFIGURATION_BONDING_CURVE_RATE',
			}),
		},
	],
};
	...
	describe('Failure cases', function () {
		tests.failure.forEach(function (failureTest) {
			it(failureTest.description, async function () {
				...
				// Execute the transaction.
				await expect(
					this.targetContract.connect(caller).deploy(owner, handle, uri, properties, metadata, [], []),
