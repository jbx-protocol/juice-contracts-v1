	...
	describe('Success cases', function () {
		tests.success.forEach(function (successTest) {
			it(successTest.description, async function () {
				...
				const terminalDirectory = await this.deployMockLocalContractFn('TerminalDirectory', [projects.address]);
				...
				await this.terminalDirectory.mock.terminalOf.withArgs(this.projectId).returns(terminalV1.address);

				await terminalV1.mock.pay.withArgs(this.projectId, beneficiary, memo, preferUnstakedTickets).returns(0);
