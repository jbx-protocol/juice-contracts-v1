	...
	describe('Success cases', function () {
		tests.success.forEach(function (successTest) {
			it(successTest.description, async function () {
				...
				const preBalance = await this.contract.provider.getBalance(this.contract.address);
				expect(preBalance).to.equal(value);
				// Execute the transaction.
				await this.contract.connect(caller).withdraw(beneficiary, value);
				const balance = await this.contract.provider.getBalance(this.contract.address);
	...
	describe('Failure cases', function () {
		tests.failure.forEach(function (failureTest) {
			it(failureTest.description, async function () {
				...
				// Execute the transaction.
				await expect(this.contract.connect(caller).withdraw(beneficiary, value)).to.be.revertedWith(revert);
			});
		});
	});
}
