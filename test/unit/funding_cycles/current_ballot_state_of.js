...
const testTemplate =
	({ set, setup = {}, preconfigure = {}, fastforward, ops = [], expectation = {}, revert }) =>
	({ deployer, ballot }) => ({
		caller: deployer,
...
const tests = {
	success: [
		...
		{
			description: 'immediately after the first funding cycle, ignoring the option to configure the active one',
			fn: testTemplate({
				preconfigure: {
					duration: BigNumber.from(1),
					configureActiveFundingCycle: true,
	...
	describe('Success cases', function () {
		tests.success.forEach(function (successTest) {
			it(successTest.description, async function () {
				...
				const storedCurrentBallotState = await this.contract.currentBallotStateOf(projectId);
	...
	describe('Failure cases', function () {
		tests.failure.forEach(function (failureTest) {
			it(failureTest.description, async function () {
				...
				await expect(this.contract.connect(caller).currentBallotStateOf(projectId)).to.be.revertedWith(revert);
			});
		});
	});
}
