...
const tests = {
	success: [
		...
		{
			description: 'during first funding cycle with duration of 0, fastforwarded a bunch',
			fn: testTemplate({
				preconfigure: {
					// Preconfigure the duration.
					duration: BigNumber.from(0),
		...
		{
			description: 'shortly after the first funding cycle, configuring the active cycle',
			fn: testTemplate({
				op: {
					// Allow the active funding cycle to be reconfigured.
					configureActiveFundingCycle: true,
		...
		},
		{
			...
			fn: testTemplate({
				op: {
					...
					currency: BigNumber.from(2).pow(8).sub(1),
		...
		{
			description: 'ballot duration from current time is before preconfigured duration',
			fn: testTemplate({
				preconfigure: {
					// Preconfigure the duration.
					duration: BigNumber.from(1),
		...
		{
			description: 'ballot duration from current time is preconfigured duration',
			fn: testTemplate({
				preconfigure: {
					// Preconfigure the duration.
					duration: BigNumber.from(1),
		...
		{
			description: 'large cycle limit, many cycles later, with a way different duration',
			fn: testTemplate({
				preconfigure: {
					// Preconfigure the duration.
					duration: BigNumber.from(10),
			...
			it(successTest.description, async function () {
				...
				await expect(tx)
					...
					.withArgs(
						...
						projectId,
						now,
						[target, currency, duration, cycleLimit, discountRate, this.ballot.address],
						metadata,
						caller.address,
					);
				...
				for (let i = 0; i < expectation.configuredNumber - 1 - discountRatesToApply.length; i += 1) {
					expectedWeight = expectedWeight.mul(BigNumber.from(1000).sub(preconfigure.discountRate)).div(1000);
				}
				for (let i = 0; i < discountRatesToApply.length; i += 1) {
					expectedWeight = expectedWeight.mul(BigNumber.from(1000).sub(discountRatesToApply[i])).div(1000);
				}
				...
				const storedFundingCycle = await this.contract.get(expectation.configuredId);
				...
				expect(storedFundingCycle.number).to.equal(expectation.configuredNumber);
