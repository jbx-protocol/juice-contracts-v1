		...
		tests.success.forEach(function (successTest) {
			it(successTest.description, async function () {
				...
				await expect(tx)
					...
					.withArgs(expectation.tappedId, projectId, amount, expectation.newTappedAmount, caller.address);
				...
				if (expectation.initNumber > 1) {
					...
					if (preconfigure) {
						expectedStart = expectedPreconfigureStart.add(
							preconfigure.duration.mul(86400).mul(expectation.initNumber - 1 - durationsToApply.length),
						);
	...
	describe('Failure cases', function () {
		tests.failure.forEach(function (failureTest) {
			it(failureTest.description, async function () {
				...
				await expect(this.contract.connect(caller).tap(projectId, amount)).to.be.revertedWith(revert);
			});
		});
	});
}
