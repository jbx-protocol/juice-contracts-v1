	...
	describe('Failure cases', function () {
		tests.failure.forEach(function (failureTest) {
			it(failureTest.description, async function () {
				const { caller, handle, setup: { create, prechallenge } = {}, revert } = failureTest.fn(this);
				...
				// Execute the transaction.
				await expect(this.contract.connect(caller).challengeHandle(handle)).to.be.revertedWith(revert);
			});
		});
	});
}
