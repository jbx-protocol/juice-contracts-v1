	...
	describe('Success cases', function () {
		tests.success.forEach(function (successTest) {
			it(successTest.description, async function () {
				const { caller, to, newHandle, setup: { create, permissionFlag } = {} } = successTest.fn(this);

				// Setup by creating a project.
				await this.contract.connect(caller).create(create.owner, create.handle, '', this.constants.AddressZero);
				...
				const tx = await this.contract.connect(caller).transferHandle(1, to, newHandle);
				...
				const storedTransferedAddress = await this.contract.transferAddressFor(create.handle);
	...
	describe('Failure cases', function () {
		tests.failure.forEach(function (failureTest) {
			it(failureTest.description, async function () {
				...
				// Execute the transaction.
				await expect(this.contract.connect(caller).transferHandle(1, to, newHandle)).to.be.revertedWith(revert);
			});
		});
	});
}
