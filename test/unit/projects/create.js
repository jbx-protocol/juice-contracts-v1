	...
	success: [
		...
		},
		{
			...
			fn: async ({ deployer, deployMockLocalContractFn }) => {
				...
				const terminalDirectory = await deployMockLocalContractFn('TerminalDirectory', [projects.address]);
				const fundingCycles = await deployMockLocalContractFn('FundingCycles', [terminalDirectory.address]);
				const ticketBooth = await deployMockLocalContractFn('TicketBooth', [
					projects.address,
					operatorStore.address,
					terminalDirectory.address,
				]);
				const modStore = await deployMockLocalContractFn('ModStore', [projects.address, operatorStore.address]);
				...
				await terminalV1.mock.terminalDirectory.withArgs().returns(terminalDirectory.address);

				// mock the set terminal function of the terminal directory.
				await terminalDirectory.mock.setTerminal.withArgs(1, terminalV1.address).returns();
	...
	describe('Success cases', function () {
		tests.success.forEach(function (successTest) {
			it(successTest.description, async function () {
				...
				const tx = await this.contract.connect(caller).create(owner, handle, uri, terminal);
	...
	describe('Failure cases', function () {
		tests.failure.forEach(function (failureTest) {
			it(failureTest.description, async function () {
				...
				// Execute the transaction.
				await expect(this.contract.connect(caller).create(owner, handle, uri, terminal)).to.be.revertedWith(
					revert,
				);
			});
		});
