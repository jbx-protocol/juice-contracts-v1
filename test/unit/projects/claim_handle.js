			...
			it(successTest.description, async function () {
				...
				if (transferToPermissionFlag !== undefined) {
					...
					await this.operatorStore.mock.hasPermission
						.withArgs(caller.address, claimFor, transferToPersonalPermission ? 2 : 0, permissionIndex)
						.returns(false);

					// Mock the caller to be the project's controller.
					await this.operatorStore.mock.hasPermission
						.withArgs(caller.address, claimFor, transferToPersonalPermission ? 0 : 2, permissionIndex)
				...
				const storedChallengeExpiryOf = await this.contract.challengeExpiryOf(handle);
	...
	describe('Failure cases', function () {
		tests.failure.forEach(function (failureTest) {
			it(failureTest.description, async function () {
				...
				// Setup by creating a project that the handle will be transfered from.
				await this.contract.connect(caller).create(source.owner, source.handle, '', this.constants.AddressZero);
				...
				// Execute the transaction.
				await expect(this.contract.connect(caller).claimHandle(handle, claimFor, 2)).to.be.revertedWith(revert);
			});
		});
	});
}
