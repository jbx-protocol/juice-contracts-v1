	...
	describe('Success cases', function () {
		tests.success.forEach(function (successTest) {
			it(successTest.description, async function () {
				const { caller, projectOwner, projectId, permissionFlag } = successTest.fn(this);
				...
				const tx = await this.contract.connect(caller).issue(projectId, name, symbol);
		...
		tests.failure.forEach(function (failureTest) {
			it(failureTest.description, async function () {
				...
				await expect(
					this.contract
						.connect(caller)
						.issue(projectId, useEmptyName ? '' : 'doesnt', useEmptySymbol ? '' : 'matter'),
