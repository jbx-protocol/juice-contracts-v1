		...
		tests.success.forEach(function (successTest) {
			it(successTest.description, async function () {
				const {
					...
					projectId,
					amount,
					setup: { permissionFlag, personalOperator, stakedBalance, lockedAmount },
				} = successTest.fn(this);
				...
				const tx = await this.contract.connect(caller).lock(holder, projectId, amount);
	...
	describe('Failure cases', function () {
		tests.failure.forEach(function (failureTest) {
			it(failureTest.description, async function () {
				...
				// Execute the transaction.
				await expect(this.contract.connect(caller).lock(holder, projectId, amount)).to.be.revertedWith(revert);
			});
		});
	});
}
