			...
			it(successTest.description, async function () {
				...
				await expect(tx)
					...
					.withArgs(
						...
						projectId,
						amount,
						stakedBalance.sub ? stakedBalance.sub(lockedAmount) : stakedBalance - lockedAmount,
						preferUnstaked,
						caller.address,
					);
				...
				if (preferUnstaked && erc20Balance > 0) {
					...
					expectedStakedTotalSupply =
						stakedBalance.sub(amount) > lockedAmount ? stakedBalance.sub(amount) : lockedAmount;
				}

				// Get the stored staked supply.
				const storedStakedTotalSupply = await this.contract.connect(caller).stakedTotalSupplyOf(projectId);
				...
				// Get the stored staked supply.
				const storedStakedBalance = await this.contract.connect(caller).stakedBalanceOf(holder, projectId);
				...
				if (erc20Balance > 0) {
					...
					if (preferUnstaked) {
						expectedTicketBalance = erc20Balance > amount ? erc20Balance.sub(amount) : 0;
					} else {
						expectedTicketBalance = erc20Balance.sub(
							amount > stakedBalance.sub(lockedAmount) ? amount.sub(stakedBalance.sub(lockedAmount)) : 0,
						);
					}
	...
	describe('Failure cases', function () {
		tests.failure.forEach(function (failureTest) {
			it(failureTest.description, async function () {
				...
				// Execute the transaction.
				await expect(
					this.contract.connect(caller).redeem(holder, projectId, amount, preferUnstaked),
