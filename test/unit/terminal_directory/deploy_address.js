			...
			it(successTest.description, async function () {
				...
				await Promise.all(
					ops.map(async (op) => {
						...
						const tx = await this.contract.connect(caller).deployAddress(op.projectId, op.memo);
				...
				const DirectPaymentAddressFactory = await ethers.getContractFactory('DirectPaymentAddress');

				expectations.forEach(async (expectation) => {
					// Get the stored addresses for the project.
					const storedAddresses = await this.contract.connect(caller).addressesOf(expectation.projectId);
					storedAddresses.forEach(async (address, i) => {
						const StoredDirectPaymentAddress = await DirectPaymentAddressFactory.attach(address);
	...
	describe('Failure cases', function () {
		tests.failure.forEach(function (failureTest) {
			it(failureTest.description, async function () {
				...
				await expect(this.contract.connect(caller).deployAddress(projectId, memo)).to.be.revertedWith(revert);
			});
		});
	});
}
