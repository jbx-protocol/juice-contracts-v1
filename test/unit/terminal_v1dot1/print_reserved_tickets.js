  ...
  success: [
    ...
    },
    {
      ...
      fn: ({ addrs }) => ({
        ...
        expectedReservedAmount: BigNumber.from(10).pow(18).mul(2),
    ...
    },
    {
      ...
      fn: () => ({
        reservedRate: 0,
        expectedReservedAmount: BigNumber.from(0),
        expectedLeftover: BigNumber.from(0)
      }),
    },
  ],
  failure: [
    {
      ...
      fn: () => ({
        ...
        revert: 'TerminalV1dot1::printReservedTickets: INT_LIMIT_REACHED',
      }),
    },
  ],
};
  ...
  ({ deployer, addrs, mockContracts, targetContract }) =>
    async (custom) => {
      const {
        ...
        totalTickets = BigNumber.from(10).pow(18).mul(18),
        mod,
        reservedRate = 20, // 10%
        expectedReservedAmount = BigNumber.from(10).pow(18).mul(2),
        expectedLeftover = BigNumber.from(10).pow(18).mul(2),
              ...
              ? [
                {
                  ...
                  args: [
                    fundingCycleId,
                    projectId,
                    [mod.preferUnstaked, mod.percent, mod.lockedUntil, mod.beneficiary],
                    expectedReservedAmount.mul(mod.percent).div(10000),
                    caller.address,
                  ],
                },
