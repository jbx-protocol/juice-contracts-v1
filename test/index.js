...
describe('Juicebox', async function () {
	before(async function () {
		...
		this.randomAddressFn = ({ exclude = [] } = {}) => {
			...
			const candidate =
				Math.random() < 0.5 ? this.addrs[0].address : this.addrs[Math.floor(Math.random() * 9)].address;
		...
		this.randomSignerFn = ({ exclude = [] } = {}) => {
			// To test an edge condition, pick the same address more likely than not.
			// return address0 50% of the time.
			const candidate = Math.random() < 0.5 ? this.addrs[0] : this.addrs[Math.floor(Math.random() * 9)];
		...
		this.randomStringFn = ({ exclude = [], prepend = '', canBeEmpty = true, favorEdges = true } = {}) => {
