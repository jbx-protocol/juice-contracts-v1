...
export default function () {
    ...
    */
		...
		this.constants.InitialWeightMultiplier = (await fundingCycles.BASE_WEIGHT()).div(BigNumber.from(10).pow(18));
	...
	for (let i = 0; i < 8; i += 1) {
		...
		describe('Projects can print premined tickets before a payment has been made to it', run(printPreminedTickets));
		describe('Issues tickets and honors preference', run(issueTickets));
		describe('Tap funds up to the configured target', run(tap));
		describe("A fee should be taken into governance's project when a project taps funds", run(takeFee));
		...
		describe('A project can be created without a payment terminal, and can set one after', run(setTerminal));
		describe(
