...
export default [
	{
		...
		fn: async ({ randomSignerFn, contracts, executeFn, randomBytesFn, randomStringFn, incrementProjectIdFn }) => {
	...
	},
	{
		...
		fn: async ({
			...
			await executeFn({
				...
				fn: 'pay',
				args: [expectedProjectId, preconfigTicketBeneficiary.address, randomStringFn(), randomBoolFn()],
				value: paymentValue1,
			});
	...
	},
	{
		...
		fn: ({ constants, contracts, checkFn, randomBigNumberFn, randomSignerFn, local: { expectedProjectId } }) =>
			checkFn({
				caller: randomSignerFn(),
				contract: contracts.terminalV1,
	...
	},
	{
		...
		fn: async ({
			...
			const expectedPreminedPrintedTicketAmount = preminePrintAmount.mul(constants.InitialWeightMultiplier);
	...
	},
	{
		...
		fn: ({ contracts, executeFn, randomStringFn, local: { expectedProjectId, owner } }) =>
			executeFn({
				...
				contract: contracts.ticketBooth,
				fn: 'issue',
				args: [expectedProjectId, randomStringFn({ canBeEmpty: false }), randomStringFn({ canBeEmpty: false })],
			}),
	},
	{
	...
	{
		description: 'The owner should now have the correct amount of tickets, accounting for any ticket mods',
		fn: ({
			constants,
			contracts,
			checkFn,
	...
	},
	{
		...
		fn: ({
			...
			checkFn,
			randomSignerFn,
			local: { expectedProjectId, expectedReservedTicketAmount2, mod1Beneficiary, preferUnstaked, percent1 },
		}) =>
			checkFn({
				caller: randomSignerFn(),
				contract: contracts.ticketBooth,
	...
	},
	{
		...
		fn: ({
			...
			checkFn,
			randomSignerFn,
			local: { expectedProjectId, expectedReservedTicketAmount2, mod2Beneficiary, preferUnstaked, percent2 },
		}) =>
			checkFn({
				caller: randomSignerFn(),
				contract: contracts.ticketBooth,
	...
	},
	{
		...
		fn: async ({
			...
			const ticketBeneficiary2 = randomSignerFn({
				exclude: [owner.address, mod1Beneficiary.address, mod2Beneficiary.address, ticketBeneficiary1.address],
			});
			await executeFn({
				caller: payer,
				contract: contracts.terminalV1,
	...
	},
	{
		...
		fn: async ({
			...
			const ticketAmount = await contracts.ticketBooth.balanceOf(ticketBeneficiary1.address, expectedProjectId);
	...
	},
	{
		...
		fn: async ({
			...
			const ticketAmount = await contracts.ticketBooth.balanceOf(ticketBeneficiary2.address, expectedProjectId);
			const claimable = await contracts.terminalV1.claimableOverflowOf(
