...
export default [
	...
	},
	{
		...
		fn: ({
			...
			checkFn,
			randomSignerFn,
			local: { payerTicketBeneficiary, expectedTicketAmount, preferUnstakedTickets, expectedProjectId },
		}) =>
			checkFn({
				caller: randomSignerFn(),
				contract: contracts.ticketBooth,
	...
	},
	{
		...
		fn: ({ contracts, executeFn, local: { owner, expectedProjectId, secondTerminalV1 } }) =>
			executeFn({
				caller: owner,
				contract: contracts.terminalV1,
				fn: 'migrate',
	...
	{
		description: 'Make another payment to the address. It should now have been routed to the new terminal',
		fn: async ({ contracts, local: { payer, paymentValue, expectedProjectId } }) => {
			const [address] = await contracts.terminalDirectory.addressesOf(expectedProjectId);
			await payer.sendTransaction({
				to: address,
	...
	},
	{
		...
		fn: ({ checkFn, randomSignerFn, local: { paymentValue, expectedProjectId, secondTerminalV1 } }) =>
			checkFn({
				caller: randomSignerFn(),
