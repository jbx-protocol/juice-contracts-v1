...
export default [
	...
	},
	{
		...
		fn: ({ executeFn, contracts, local: { expectedProjectId, ticketBeneficiary, amountToUnstake } }) =>
			executeFn({
				caller: ticketBeneficiary,
				contract: contracts.ticketBooth,
				fn: 'unstake',
	...
	},
	{
		...
		fn: async ({
			...
			await executeFn({
				...
				contract: contracts.ticketBooth,
				fn: 'transfer',
				args: [ticketBeneficiary.address, expectedProjectId, amountToTransfer, ticketTransferRecipient.address],
				revert:
	...
	},
	{
		...
		fn: ({
			...
			executeFn({
				...
				revert:
					(amountToTransfer.eq(0) || ticketBeneficiary.address !== ticketTransferRecipient.address) &&
	...
	},
	{
		...
		fn: async ({
			...
			const ticketsToRedeem = await contracts.ticketBooth.balanceOf(ticketBeneficiary.address, expectedProjectId);
	...
	},
	{
		...
		fn: async ({
			...
			await executeFn({
				...
				fn: 'unlock',
				args: [ticketBeneficiary.address, expectedProjectId, amountToLock],
				revert: amountToLock.eq(0) ? 'TicketBooth::unlock: NO_OP' : 'TicketBooth::unlock: INSUFFICIENT_FUNDS',
			});
		},
	},
	{
	...
	},
	{
		...
		fn: async ({
			...
			randomAddressFn,
			randomBoolFn,
			local: { expectedProjectId, ticketBeneficiary, ticketsToRedeem, amountToLock, expectedClaimedAmountIsZero },
		}) => {
			await executeFn({
				caller: ticketBeneficiary,
				contract: contracts.terminalV1,
				...
				revert:
					ticketsToRedeem.eq(0) || expectedClaimedAmountIsZero
						...
						: // If the locked amount is zero, the tickets have already been redeemed.
						  amountToLock.eq(0) && 'TerminalV1::claimableOverflow: INSUFFICIENT_TICKETS',
			});
		},
	},
	{
		description: 'The balance should be 0',
		fn: ({ checkFn, randomSignerFn, contracts, BigNumber, local: { expectedProjectId, ticketBeneficiary } }) =>
			checkFn({
				caller: randomSignerFn(),
				contract: contracts.ticketBooth,
				fn: 'balanceOf',
	...
	{
		description: 'The staked balance should be zero, or with a small margin of error caused by division rounding',
		fn: ({ checkFn, randomSignerFn, contracts, BigNumber, local: { expectedProjectId, ticketBeneficiary } }) =>
			checkFn({
				caller: randomSignerFn(),
				contract: contracts.ticketBooth,
