...
export default [
  ...
  },
  {
    ...
    fn: async ({
      ...
      return { ticketBeneficiary, initialTerminalV1_1Balance };
    },
  },
  {
    description: "The terminal's balance should match the payment just made",
    fn: ({ contracts, verifyBalanceFn, local: { paymentValue1, initialTerminalV1_1Balance } }) =>
      verifyBalanceFn({
        address: contracts.terminalV1_1.address,
        expect: initialTerminalV1_1Balance.add(paymentValue1),
      }),
  },
  {
    description: 'Make sure tickets can be redeemed successfully in this terminalV1_1',
    fn: async ({
      deployer,
      contracts,
      executeFn,
  ...
  {
    description: 'Make sure funds can be tapped successfully in this terminalV1_1',
    fn: async ({
      contracts,
      BigNumber,
      executeFn,
  ...
  {
    description: "Migrating to a new terminalV1_1 shouldn't work because it hasn't been allowed yet",
  ...
  {
    description: 'Allow a migration to the new terminalV1_1',
    fn: ({ multisig, contracts, executeFn, local: { secondTerminalV1_1 } }) =>
      executeFn({
        caller: multisig,
        contract: contracts.terminalV1_1,
        fn: 'allowMigration',
  ...
  {
    description:
      "Migrating to the new terminalV1_1 called by a different address shouldn't be allowed",
    fn: ({
      contracts,
      executeFn,
      randomSignerFn,
      local: { owner, expectedProjectId, secondTerminalV1_1, redeemBeneficiary },
    }) =>
      executeFn({
  ...
  {
    description:
      'Migrate to the new terminalV1_1, which should automatically print reserved tickets for the owner',
    fn: async ({
      contracts,
      executeFn,
      local: { owner, expectedProjectId, secondTerminalV1_1, reservedRate },
    }) => {
      // Before migrating, save a reference to the amount of reserved tickets available.
      const reservedTicketAmount = await contracts.terminalV1_1.reservedTicketBalanceOf(
        expectedProjectId,
        reservedRate,
      );
      await executeFn({
  ...
  {
    description:
      'The only balance that should be left in the old terminalV1_1 is the admin fee incurred while tapping',
    fn: async ({
      constants,
      contracts,
      verifyBalanceFn,
  ...
  },
  {
    ...
    fn: async ({
      ...
      verifyBalanceFn({
        address: secondTerminalV1_1.address,
  ...
  {
    description: 'The terminal should have been updated to the new terminalV1_1 in the directory',
    fn: ({ contracts, checkFn, randomSignerFn, local: { expectedProjectId, secondTerminalV1_1 } }) =>
      checkFn({
        caller: randomSignerFn(),
        contract: contracts.terminalDirectory,
        fn: 'terminalOf',
        args: [expectedProjectId],
        expect: secondTerminalV1_1.address,
      }),
  },
  {
    description: 'Payments to the old TerminalV1_1 should no longer be accepted',
    fn: ({
      contracts,
      executeFn,
      randomAddressFn,
  ...
  {
    description: 'Make sure funds can be tapped successfully in the new TerminalV1_1',
    fn: ({
      executeFn,
      randomSignerFn,
  ...
  {
    description: 'Make sure tickets can be redeemed successfully in the new TerminalV1_1',
    fn: ({
      executeFn,
      randomAddressFn,
      randomBoolFn,
  ...
  },
  {
    ...
    fn: ({
      ...
      randomAddressFn,
      randomBoolFn,
      local: { reservedTicketAmount, owner, reservedRate, expectedProjectId, secondTerminalV1_1 },
    }) =>
      executeFn({
        caller: owner,
        contract: secondTerminalV1_1,
  ...
  {
    description: 'Payments to the new TerminalV1_1 should be accepted',
    fn: ({
      executeFn,
      randomAddressFn,
      randomBoolFn,
      randomStringFn,
      local: { payer, paymentValue2, expectedProjectId, secondTerminalV1_1 },
    }) =>
      executeFn({
        caller: payer,
        contract: secondTerminalV1_1,
