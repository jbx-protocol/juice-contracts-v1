...
export default [
	...
	},
	{
		...
		fn: async ({
			...
			const targetDenominatedInCurrency = targetDenominatedInWei.mul(rate.div(BigNumber.from(10).pow(decimals)));
	...
	},
	{
		...
		fn: async ({
			...
			const premineValueInCurrency = premineValueInWei.mul(rate.div(BigNumber.from(10).pow(decimals)));
			await executeFn({
				caller: owner,
				contract: contracts.terminalV1,
	...
	},
	{
		...
		fn: async ({
			...
			const amountToTapInCurrency = amountToTapInWei.mul(rate.div(BigNumber.from(10).pow(decimals)));
