...
export default [
	...
	},
	{
		...
		fn: async ({
			...
			await checkFn({
				...
				expect: [
					[lockedMod.preferUnstaked, lockedMod.percent, lockedMod.lockedUntil, lockedMod.beneficiary],
					[
	...
	{
		description: "Overriding a locked mod with a shorter locked date shouldn't work when setting ticket mods",
		fn: ({
			contracts,
			executeFn,
			timeMark,
	...
	},
	{
		...
		fn: ({ contracts, executeFn, timeMark, local: { owner, expectedProjectId, lockedMod, unlockedMod2 } }) =>
			executeFn({
				caller: owner,
				contract: contracts.modStore,
				fn: 'setTicketMods',
	...
	},
	{
		...
		fn: ({
			...
			checkFn({
				...
				expect: [
					[lockedMod.preferUnstaked, lockedMod.percent, lockedMod.lockedUntil + 1, lockedMod.beneficiary],
					[
	...
	{
		description: 'Configuring a project should allow overriding locked mods for the new configuration',
		fn: ({
			constants,
			contracts,
			executeFn,
	...
	},
	{
		...
		fn: ({ contracts, checkFn, randomSignerFn, timeMark, local: { expectedProjectId, unlockedMod1 } }) =>
			checkFn({
				caller: randomSignerFn(),
				contract: contracts.modStore,
				fn: 'ticketModsOf',
