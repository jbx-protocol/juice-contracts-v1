...
export default [
	...
	{
		description: "Migrating to the new terminalV1 called by a different address shouldn't be allowed",
		fn: ({
			contracts,
			executeFn,
			randomSignerFn,
	...
	{
		description: 'Migrate to the new terminalV1, which should automatically print reserved tickets for the owner',
		fn: async ({ contracts, executeFn, local: { owner, expectedProjectId, secondTerminalV1, reservedRate } }) => {
	...
	},
	{
		...
		fn: async ({ constants, contracts, verifyBalanceFn, local: { amountToTap1, initialTerminalV1Balance } }) => {
	...
	},
	{
		...
		fn: ({ executeFn, randomSignerFn, local: { target, expectedProjectId, amountToTap1, secondTerminalV1 } }) =>
			executeFn({
				caller: randomSignerFn(),
