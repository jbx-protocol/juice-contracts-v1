...
export default [
	...
	},
	{
		...
		fn: ({ contracts, executeFn, randomStringFn, local: { modProjectOwner, expectedIdOfModProject } }) =>
			executeFn({
	...
	},
	{
		...
		fn: ({
			verifyBalanceFn,
			constants,
			local: { owner, addressMod, projectMod, allocatorMod, expectedAmountTapped, ownerInitialBalance },
		}) =>
			verifyBalanceFn({
	...
	},
	{
		...
		fn: ({
			...
			checkFn,
			randomSignerFn,
			local: { owner, amountToTap, expectedAmountTapped, initialOwnerTicketBalanceOfGovernanceProject },
		}) =>
			checkFn({
				caller: randomSignerFn(),
				contract: contracts.ticketBooth,
	...
	},
	{
		...
		fn: ({ contracts, executeFn, randomSignerFn, local: { expectedIdOfBaseProject, paymentValue2 } }) =>
			executeFn({
				caller: randomSignerFn(),
				contract: contracts.terminalV1,
				fn: 'tap',
