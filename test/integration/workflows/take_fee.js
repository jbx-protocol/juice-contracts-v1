...
export default [
	...
	},
	{
		...
		fn: async ({
			...
			const amountToTap1 = target.sub(randomBigNumberFn({ min: BigNumber.from(1), max: target.sub(1) }));

			// Save the initial balances of the owner, address mod beneficiary, and the allocator mod contract.
			const governanceInitialBalance = await contracts.terminalV1.balanceOf(constants.GovernanceProjectId);
