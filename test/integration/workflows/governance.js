...
export default [
	...
	},
	{
		...
		fn: ({ executeFn, contracts, randomBigNumberFn, constants, local: { firstAppointedGovernance } }) =>
			executeFn({
	...
	{
		description: "If they're different, the first appointed governance should no longer be able to accept",
		fn: ({ executeFn, contracts, local: { firstAppointedGovernance, secondAppointedGovernance } }) =>
			executeFn({
				caller: firstAppointedGovernance,
				contract: contracts.terminalV1,
				fn: 'acceptGovernance',
	...
	},
	{
		...
		fn: ({ executeFn, deployer, contracts, randomBigNumberFn, constants, local: { secondAppointedGovernance } }) =>
			executeFn({
				caller: deployer,
				contract: contracts.governance,
				fn: 'setFee',
				args: [contracts.terminalV1.address, randomBigNumberFn({ max: constants.MaxPercent })],
				revert:
					contracts.governance.address !== secondAppointedGovernance.address && 'TerminalV1: UNAUTHORIZED',
			}),
	},
	{
		description: 'The new governance should be able to set a fee',
		fn: ({ executeFn, randomBigNumberFn, constants, contracts, local: { secondAppointedGovernance } }) =>
			executeFn({
				caller: secondAppointedGovernance,
				contract: contracts.terminalV1,
				fn: 'setFee',
