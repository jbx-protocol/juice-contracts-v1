...
export default [
	...
	{
		description: 'Make a payment to the project without first issueing tickets should print staked tickets',
		fn: async ({
			randomSignerFn,
			executeFn,
			contracts,
	...
	},
	{
		...
		fn: async ({
			...
			return {
				redeemedPortionOfStakedBalance: expectNoOp ? BigNumber.from(0) : redeemedPortionOfStakedBalance,
			};
		},
	},
	{
		...
		fn: ({
			...
			randomSignerFn,
			contracts,
			local: { ticketBeneficiary, expectedProjectId, expectedStakedBalance, redeemedPortionOfStakedBalance },
		}) =>
			checkFn({
				caller: randomSignerFn(),
				contract: contracts.ticketBooth,
	...
	},
	{
		...
		fn: ({
			...
			randomSignerFn,
			contracts,
			local: { redeemedPortionOfStakedBalance, ticketBeneficiary, expectedProjectId, expectedTotalTicketBalance },
		}) =>
			checkFn({
				caller: randomSignerFn(),
				contract: contracts.ticketBooth,
	...
	},
	{
		...
		fn: async ({
			...
			contracts,
			randomAddressFn,
			local: { expectedTotalTicketBalance, expectedStakedBalance, ticketBeneficiary, expectedProjectId },
		}) => {
			...
			return {
				redeemedPortionOfUnstakedBalance: expectNoOp ? BigNumber.from(0) : redeemedPortionOfUnstakedBalance,
			};
		},
	},
	{
		...
		fn: ({
			...
			randomSignerFn,
			contracts,
			local: { redeemedPortionOfStakedBalance, ticketBeneficiary, expectedProjectId, expectedStakedBalance },
		}) =>
			checkFn({
				caller: randomSignerFn(),
				contract: contracts.ticketBooth,
	...
	},
	{
		...
		fn: async ({
			...
			const balance = await contracts.ticketBooth.balanceOf(ticketBeneficiary.address, expectedProjectId);
