...
export default [
	{
		...
		fn: async ({ executeFn, randomStringFn, contracts, randomBytesFn, randomSignerFn, incrementProjectIdFn }) => {
	...
	},
	{
		...
		fn: async ({
			...
			const expectedPreminedPrintedTicketAmount1 = preminePrintAmount1.mul(constants.InitialWeightMultiplier);
			await checkFn({
				caller: randomSignerFn(),
	...
	},
	{
		...
		fn: ({
			...
			checkFn,
			contracts,
			local: { expectedProjectId, preconfigureTicketBeneficiary1, expectedPreminedPrintedTicketAmount1 },
		}) =>
			checkFn({
				caller: randomSignerFn(),
				contract: contracts.ticketBooth,
	...
	},
	{
		...
		fn: async ({
			...
			await executeFn({
				...
				contract: contracts.terminalV1,
				fn: 'pay',
				args: [expectedProjectId, preconfigureTicketBeneficiary2.address, randomStringFn(), randomBoolFn()],
				value: paymentValue1,
			});
	...
	},
	{
		...
		fn: async ({
			...
			// The ticket amount is based on the initial funding cycle's weight.
			const expectedPaymentPrintedTicketAmount1 = paymentValue1.mul(constants.InitialWeightMultiplier);
			await checkFn({
				caller: randomSignerFn(),
				contract: contracts.ticketBooth,
				fn: 'balanceOf',
	...
	},
	{
		...
		fn: async ({
			...
			const expectedPreminedPrintedTicketAmount2 = preminePrintAmount2.mul(constants.InitialWeightMultiplier);
	...
	},
	{
		...
		fn: async ({
			...
			let expectedStaked = preferUnstakedTickets ? BigNumber.from(0) : expectedPreminedPrintedTicketAmount2;
	...
	{
		description: "Make a second payment to lock in the premined amount now that there's a configured funding cycle",
		fn: async ({
			randomBigNumberFn,
			BigNumber,
			getBalanceFn,
