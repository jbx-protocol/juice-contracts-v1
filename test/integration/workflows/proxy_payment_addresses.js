...
export default [
	...
	},
	{
		...
		fn: async ({ contracts, BigNumber, randomSignerFn, local: { expectedProjectId } }) => {
			const [proxyPaymentAddress] = await contracts.proxyPaymentAddressManager.addressesOf(expectedProjectId);
	...
	{
		description: 'Make sure the correct number of tickets were printed for the proxy payment address',
		fn: ({
			checkFn,
			contracts,
			constants,
	...
	},
	{
		...
		fn: async ({ randomSignerFn, executeFn, local: { proxyPaymentAddress, expectedNumTickets } }) => {
	...
	},
	{
		...
		fn: async ({ randomSignerFn, deployer, executeFn, local: { proxyPaymentAddress, expectedNumTickets } }) => {
			const beneficiary = randomSignerFn();
	...
	},
	{
		...
		fn: ({ checkFn, contracts, randomSignerFn, local: { expectedNumTickets, expectedProjectId, beneficiary } }) => {
			checkFn({
				caller: randomSignerFn(),
				contract: contracts.ticketBooth,
				fn: 'balanceOf',
