...
export default [
  {
    ...
    fn: async ({
      ...
      await executeFn({
        ...
        contract: contracts.terminalV1dot1,
        fn: 'deploy',
        args: [
          owner.address,
          randomBytesFn({
  ...
  },
  {
    ...
    fn: ({ contracts, checkFn, randomSignerFn, local: { expectedProjectId } }) =>
      checkFn({
        ...
        expect: contracts.terminalV1dot1.address,
      }),
  },
  {
    ...
    fn: async ({
      ...
      const initialTerminalV1dot1Balance = await getBalanceFn(contracts.terminalV1dot1.address);

      console.log({ initialTerminalV1dot1Balance });
      await executeFn({
        caller: payer,
        contract: contracts.terminalV1dot1,
        fn: 'pay',
      ...
      return { ticketBeneficiary, initialTerminalV1dot1Balance };
    },
  },
  {
    description: "The terminal's balance should match the payment just made",
    fn: ({ contracts, verifyBalanceFn, local: { paymentValue1, initialTerminalV1dot1Balance } }) =>
      verifyBalanceFn({
        address: contracts.terminalV1dot1.address,
        expect: initialTerminalV1dot1Balance.add(paymentValue1),
      }),
  },
  {
    description: 'Make sure tickets can be redeemed successfully in this terminalV1dot1',
    fn: async ({
      deployer,
      contracts,
      executeFn,
  ...
  {
    description: 'Make sure funds can be tapped successfully in this terminalV1dot1',
    fn: async ({
      contracts,
      BigNumber,
      executeFn,
  ...
  {
    description: "Migrating to a new terminalV1dot1 shouldn't work because it hasn't been allowed yet",
    fn: async ({ contracts, executeFn, deployContractFn, local: { owner, expectedProjectId } }) => {
      // The terminalV1dot1 that will be migrated to.
      const secondTerminalV1dot1 = await deployContractFn('TerminalV1dot1', [
      ...
      await executeFn({
        ...
        contract: contracts.terminalV1dot1,
        fn: 'migrate',
        args: [expectedProjectId, secondTerminalV1dot1.address],
        revert: 'TerminalV1dot1::migrate: NOT_ALLOWED',
      });

      return { secondTerminalV1dot1 };
    },
  },
  {
    description: 'Allow a migration to the new terminalV1dot1',
    fn: ({ deployer, contracts, executeFn, local: { secondTerminalV1dot1 } }) =>
      executeFn({
        caller: deployer,
        contract: contracts.governance,
        fn: 'allowMigration',
        args: [contracts.terminalV1dot1.address, secondTerminalV1dot1.address],
      }),
  },
  {
    description:
      "Migrating to the new terminalV1dot1 called by a different address shouldn't be allowed",
    fn: ({
      contracts,
      executeFn,
      randomSignerFn,
      local: { owner, expectedProjectId, secondTerminalV1dot1, redeemBeneficiary },
    }) =>
      executeFn({
  ...
  {
    description:
      'Migrate to the new terminalV1dot1, which should automatically print reserved tickets for the owner',
    fn: async ({
      contracts,
      executeFn,
      local: { owner, expectedProjectId, secondTerminalV1dot1, reservedRate },
    }) => {
      // Before migrating, save a reference to the amount of reserved tickets available.
      const reservedTicketAmount = await contracts.terminalV1dot1.reservedTicketBalanceOf(
        expectedProjectId,
        reservedRate,
      );
      await executeFn({
  ...
  {
    description:
      'The only balance that should be left in the old terminalV1dot1 is the admin fee incurred while tapping',
    fn: async ({
      constants,
      contracts,
      verifyBalanceFn,
      local: { amountToTap1, initialTerminalV1dot1Balance },
    }) => {

      console.log({ initialTerminalV1dot1Balance, amountToTap1 })

      await verifyBalanceFn({
        address: contracts.terminalV1dot1.address,
        expect: initialTerminalV1dot1Balance
  ...
  {
    description: 'The rest of the balance should be entirely in the new TerminalV1dot1',
    fn: async ({
      verifyBalanceFn,
      getBalanceFn,
      local: {
        ...
        redeemBeneficiary,
        amountToTap1,
        secondTerminalV1dot1,
        initialBalanceOfRedeemBeneficiary,
      },
    }) =>
      verifyBalanceFn({
        address: secondTerminalV1dot1.address,
  ...
  {
    description: 'The terminal should have been updated to the new terminalV1dot1 in the directory',
    fn: ({ contracts, checkFn, randomSignerFn, local: { expectedProjectId, secondTerminalV1dot1 } }) =>
      checkFn({
        caller: randomSignerFn(),
        contract: contracts.terminalDirectory,
        fn: 'terminalOf',
        args: [expectedProjectId],
        expect: secondTerminalV1dot1.address,
      }),
  },
  {
    description: 'Payments to the old TerminalV1dot1 should no longer be accepted',
    fn: ({
      contracts,
      executeFn,
      randomAddressFn,
  ...
  {
    description: 'Make sure funds can be tapped successfully in the new TerminalV1dot1',
    fn: ({
      executeFn,
      randomSignerFn,
      local: { target, expectedProjectId, amountToTap1, secondTerminalV1dot1 },
    }) =>
      executeFn({
        caller: randomSignerFn(),
        contract: secondTerminalV1dot1,
        fn: 'tap',
        args: [expectedProjectId, target.sub(amountToTap1), currency, target.sub(amountToTap1)],
      }),
  },
  {
    description: 'Make sure tickets can be redeemed successfully in the new TerminalV1dot1',
    fn: ({
      executeFn,
      randomAddressFn,
      randomBoolFn,
  ...
  },
  {
    ...
    fn: ({
      ...
      randomAddressFn,
      randomBoolFn,
      local: { reservedTicketAmount, owner, reservedRate, expectedProjectId, secondTerminalV1dot1 },
    }) =>
      executeFn({
        caller: owner,
        contract: secondTerminalV1dot1,
        ...
        revert: reservedRate.eq(0) && 'TerminalV1dot1::redeem: NO_OP',
      }),
  },
  {
    description: 'Payments to the new TerminalV1dot1 should be accepted',
    fn: ({
      executeFn,
      randomAddressFn,
      randomBoolFn,
      randomStringFn,
      local: { payer, paymentValue2, expectedProjectId, secondTerminalV1dot1 },
    }) =>
      executeFn({
        caller: payer,
        contract: secondTerminalV1dot1,
