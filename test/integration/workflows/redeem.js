...
export default [
	...
	},
	{
		...
		fn: async ({
			...
			randomSignerFn,
			contracts,
			local: { payer, expectedProjectId, paymentValue3, owner, ticketBeneficiary1, ticketBeneficiary2 },
		}) => {
	...
	},
	{
		...
		fn: ({
			...
			checkFn,
			randomSignerFn,
			local: { expectedProjectId, ticketBeneficiary1, expectedRedeemableTicketsOfTicketBeneficiary1 },
		}) =>
			checkFn({
	...
	},
	{
		...
		fn: async ({
			...
			await checkFn({
				...
				args: [ticketBeneficiary1.address, expectedProjectId, redeemableTicketsOfTicketBeneficiary1],
	...
	},
	{
		...
		fn: ({
			verifyBalanceFn,
			local: { redeemableAmountOfTicketBeneficiary1, redeemBeneficiary1, initialBalanceOfRedeemBeneficiary1 },
		}) =>
			verifyBalanceFn({
	...
	{
		description: 'The terminalV1 should no longer have the funds sent to the first redeem beneificiary',
		fn: ({
			contracts,
			verifyBalanceFn,
			local: {
	...
	},
	{
		...
		fn: async ({
			...
			randomSignerFn,
			constants,
			local: { reservedRate, expectedProjectId, ticketBeneficiary2, expectedTotalTicketsFromPayment2 },
		}) => {
	...
	},
	{
		...
		fn: async ({
			...
			await checkFn({
				...
				contract: contracts.terminalV1,
				fn: 'claimableOverflowOf',
				args: [ticketBeneficiary2.address, expectedProjectId, redeemableTicketsOfTicketBeneficiary2],
				expect: bondingCurveFn({
	...
	},
	{
		...
		fn: ({
			verifyBalanceFn,
			local: { redeemableAmountOfTicketBeneficiary2, redeemBeneficiary2, initialBalanceOfRedeemBeneficiary2 },
		}) =>
			verifyBalanceFn({
	...
	{
		description: 'The terminalV1 should no longer have the funds sent to the second redeem beneificiary',
		fn: ({
			contracts,
			verifyBalanceFn,
			local: {
	...
	},
	{
		...
		fn: async ({
			...
			randomSignerFn,
			constants,
			local: { reservedRate, expectedProjectId, ticketBeneficiary3, expectedTotalTicketsFromPayment3 },
		}) => {
	...
	},
	{
		...
		fn: async ({ fastforwardFn, local: { ballot } }) => fastforwardFn((await ballot.duration()).add(1)),
	},
	{
		...
		fn: async ({
			...
			await checkFn({
				...
				contract: contracts.terminalV1,
				fn: 'claimableOverflowOf',
				args: [ticketBeneficiary3.address, expectedProjectId, redeemableTicketsOfTicketBeneficiary3],
				expect: bondingCurveFn({
					rate: bondingCurveRate,
	...
	},
	{
		...
		fn: ({
			verifyBalanceFn,
			local: { redeemableAmountOfTicketBeneficiary3, redeemBeneficiary3, initialBalanceOfRedeemBeneficiary3 },
		}) =>
			verifyBalanceFn({
	...
	},
	{
		...
		fn: async ({
			...
			const reservedTicketBalance = await contracts.ticketBooth.balanceOf(owner.address, expectedProjectId);
