...
export default [
	...
	},
	{
		...
		fn: async ({ contracts, constants, executeFn, randomStringFn, randomSignerFn, local: { handle } }) => {
	...
	},
	{
		...
		fn: async ({ contracts, executeFn, randomBytesFn, local: { owner, handle, expectedProjectId } }) => {
	...
	{
		description: "Make sure the other owner can't set its project's handle to the one currently in use",
	...
	},
	{
		...
		fn: ({ deployer, contracts, constants, executeFn, randomStringFn, local: { secondOwner, secondHandle } }) =>
			executeFn({
				caller: deployer,
				contract: contracts.projects,
				fn: 'create',
	...
	},
	{
		...
		fn: ({ contracts, executeFn, local: { owner, secondOwner, secondHandle, expectedProjectId } }) =>
			executeFn({
				caller: owner,
				contract: contracts.projects,
				fn: 'claimHandle',
	...
	},
	{
		...
		fn: ({ contracts, executeFn, local: { owner, secondOwner, secondHandle, expectedSecondProjectId } }) =>
			executeFn({
				caller: secondOwner,
				contract: contracts.projects,
				fn: 'claimHandle',
	...
	},
	{
		...
		fn: ({ contracts, checkFn, randomSignerFn, local: { owner, secondOwner, handle, expectedSecondProjectId } }) =>
			checkFn({
				caller: randomSignerFn(),
				contract: contracts.projects,
				fn: 'projectFor',
	...
	},
	{
		...
		fn: ({ contracts, executeFn, randomSignerFn, local: { paymentValue, currency, expectedProjectId } }) =>
			executeFn({
				caller: randomSignerFn(),
				contract: contracts.terminalV1,
				fn: 'tap',
