...
const operations = [
	{
		expand: ({ BigNumber, contracts, constants, local: { expectedProjectId } }) => ({
			...
			permissionIndex: 1,
			authorizedRevert: 'TerminalV1::_validateAndPackFundingCycleMetadata: BAD_RESERVED_RATE',
		}),
	},
	{
		expand: ({ contracts, BigNumber, constants, local: { expectedProjectId } }) => ({
			...
			args: [expectedProjectId, BigNumber.from(1000), BigNumber.from(0), constants.AddressZero, '', false],
			domain: expectedProjectId,
			permissionIndex: 2,
	...
	},
	{
		expand: ({ contracts, BigNumber, constants, local: { expectedProjectId, owner } }) => ({
			contract: contracts.terminalV1,
			fn: 'redeem',
			args: [owner.address, expectedProjectId, BigNumber.from(1), BigNumber.from(0), constants.AddressZero, true],
			domain: expectedProjectId,
			permissionIndex: 3,
	...
	},
	{
		override: [
			...
			},
			{
				...
				fn: ({ checkFn, contracts, randomSignerFn, local: { owner, expectedProjectId, operator } }) =>
					checkFn({
			...
			},
			{
				...
				fn: async ({ executeFn, contracts, randomSignerFn, local: { expectedProjectId, operator, owner } }) => {
			...
			},
			{
				...
				fn: ({ checkFn, contracts, randomSignerFn, local: { expectedProjectId, operator, claimer } }) =>
					checkFn({
						caller: randomSignerFn(),
						contract: contracts.operatorStore,
						fn: 'hasPermission',
			...
			},
			{
				...
				fn: ({ executeFn, contracts, randomBytesFn, local: { expectedProjectId, operator, claimer } }) =>
					executeFn({
						caller: operator,
						contract: contracts.projects,
						fn: 'claimHandle',
			...
			...(override || [
				{
					...
					fn: ({ checkFn, contracts, randomSignerFn, local: { operator, domain, permissionIndex, owner } }) =>
						checkFn({
							caller: randomSignerFn(),
							contract: contracts.operatorStore,
							fn: 'hasPermission',
				...
				{
					description: 'The operator should now have permission',
					fn: ({ checkFn, randomSignerFn, contracts, local: { operator, domain, owner, permissionIndex } }) =>
						checkFn({
							caller: randomSignerFn(),
							contract: contracts.operatorStore,
							fn: 'hasPermission',
				...
				{
					description: 'A non-owner or operator account shouldnt be authorized',
					fn: ({ executeFn, randomSignerFn, local: { contract, fn, args, revert, owner, operator } }) =>
						executeFn({
							caller: randomSignerFn({
								exclude: [owner.address, operator.address],
							}),
